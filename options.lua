...
FindA.Options.InitInterfaceOptions = function()
    ...
    FindA.Options.panel.main_title : SetFont(FindA.Options.panel.main_title:GetFont(), 128, "OUTLINE")
    ...
    FindA.Options.panel.sub_title : SetFont(FindA.Options.panel.sub_title:GetFont(), 32, "OUTLINE")
    FindA.Options.panel.sub_title : ClearAllPoints()
    FindA.Options.panel.sub_title : SetPoint("TOP", 0, -72)
    ...
    -- FindA.Options.panel.background_texture : SetTexture(FindA.Constants.gradient_texture)
    ...
    FindA.Options.panel.checkboxes.show_chat_messages : SetPoint("CENTER", -250, 20)
    ...
    FindA.Options.panel.checkboxes.always_hide_macro_button : SetPoint("CENTER", -250, -20)
    ...
    FindA.Options.panel.checkboxes.remember_finding : SetPoint("CENTER", -250, -60)
    ...
    FindA.Options.panel.marker_selection.text : SetFont(FindA.Options.panel.marker_selection.text:GetFont(), 12, "OUTLINE")
    FindA.Options.panel.marker_selection.text : SetTextColor(1, 1, 1, 1)
    FindA.Options.panel.marker_selection.text : ClearAllPoints()
    FindA.Options.panel.marker_selection.text : SetPoint("TOP", -205, -170)
    ...
    FindA.Options.panel.marker_selection.menu : SetPoint("CENTER", -200, 80)
    ...
    FindA.Options.panel.checkboxes.show_chat_messages:SetChecked(FindA.chat_msg_enable == 1)
    ...
    FindA.Options.panel.checkboxes.always_hide_macro_button:SetChecked(FindA.always_hide == 1)
    ...
    FindA.Options.panel.checkboxes.remember_finding:SetChecked(FindA.remember_finding == 1)
    ...
    UIDropDownMenu_Initialize(FindA.Options.panel.marker_selection.menu, function()
        ...
        for index, marker in ipairs(FindA.Constants.markers) do
    ...
    function FindA.Options.panel.okay()
        local last_marker = FindA.Constants.markers[FindASV.marker].name
        FindASV.marker = FindA.marker
        FindASV.always_hide = FindA.always_hide
        FindASV.chat_msg_enable = FindA.chat_msg_enable
        FindASV.remember_finding = FindA.remember_finding
        ...
        if FindA.marker then
            ...
            if selected_marker ~= last_marker then
                if FindA.chat_msg_enable == 1 then
                    print("|cff00ff00Finda:  |r Marker set to  " .. FindA.Constants.markers[FindA.marker].texture)
                end
            end
        end
    end
